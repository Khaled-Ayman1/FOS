	...
	if(isPageReplacmentAlgorithmLRU(PG_REP_LRU_LISTS_APPROX))
	{
		...
		if((ALSize + SLSize) < (curenv->page_WS_max_size))//one or both of the lists are not full
		{
			...
			if(ALSize < (curenv->ActiveListSize))
			{
				...
				{
					if(ROUNDDOWN(element_to_find->virtual_address,PAGE_SIZE) == ROUNDDOWN(new_element->virtual_address,PAGE_SIZE))
					{
						flag = 1;
						break;
					}
				...
				}else
				{
					...
					map_frame(curenv->env_page_directory,new_frame,v_add,PERM_WRITEABLE | PERM_USER);
			...
			}else
			{
				...
				if(flag)
				{
					...
					LIST_REMOVE(&(curenv->SecondList),element_to_find);
		...
		else //both lists are full
		{
			...
			{
				//remove element from the place its is already in then add it at the head
				LIST_REMOVE(&(curenv->ActiveList),element_to_find);
				LIST_INSERT_HEAD(&(curenv->ActiveList),new_element);
			}
